/**
 * MatchpoolView — Globale bedrijvendatabase voor super-admin (Tendermatch)
 * TenderZen v1.0
 *
...
class MatchpoolView {
    ...
    _renderSkeleton() {
        ...
        <div class="bv-sa-container">
            ...
            <div class="bv-sa-header">
                <h1 class="bv-sa-title">Matchpool</h1>
    ...
    async _laadBedrijven() {
        ...
        try {
            ...
            console.error('[MatchpoolView] Fout laden:', err);
            if (tbody) {
    ...
    _openKoppelenModal(bedrijfId) {
        console.log('[MatchpoolView] Koppelen modal:', bedrijfId);
    ...
    _kvkOpzoeken(bedrijfId, naam, plaats) {
        console.log('[MatchpoolView] KvK opzoeken:', naam, plaats);
        window.ConfirmDialog?.show({
            titel: 'Binnenkort beschikbaar',
                ...
                <div id="bv-imp-stap-voortgang" style="display:none">
                    <div class="bv-imp-voortgang-wrap">
                        ...
                        <div class="bv-imp-vg-stats">
                            ...
                            <span>${this._icon('x', 13, '#94a3b8')} <span id="bv-imp-vg-overgeslagen">0</span> overgeslagen</span>
                            <span>${this._icon('warning', 13, '#ef4444')} <span id="bv-imp-vg-fouten">0</span> fouten</span>
                        </div>
                    </div>
                </div>
    ...
    async _startImport() {
        ...
        try {
            const resp = await fetch(`${baseURL}/api/v1/bedrijven/import/stream`, {
                method: 'POST',
                headers: {
            ...
            console.error('[MatchpoolView] Import fout:', err);
...
window.MatchpoolView = MatchpoolView;
